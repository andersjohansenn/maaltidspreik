...
// Extract simple negation terms from the user question (e.g. "without chicken", "no milk", "uten egg")
function extractNegations(question) {
  const q = (question || "").toLowerCase();
  const terms = new Set();

  const patterns = [
    /\bwithout\s+([a-zA-Zøæåäöéèåæ]+)/g,  // "without chicken"
    /\bno\s+([a-zA-Zøæåäöéèåæ]+)/g,       // "no chicken"
    /\buten\s+([a-zA-Zøæåäöéèåæ]+)/g      // Norwegian: "uten kylling"
  ];
...
async function answer(question, lucky=false) {
  ...
  // 2) Retrieve locally
  const originalTop = topK(embs, dim, qvec, lucky ? 8 : 12); // ask for a few more so we can filter
  const banned = extractNegations(question);

  let filteredTop = originalTop;

  if (banned.length > 0) {
    filteredTop = originalTop.filter(([i]) => {
      const m = meta[i] || {};
      const text = ((m.text || "") + " " + (m.title || "")).toLowerCase();
      return !banned.some(term => text.includes(term));
    });

    // If we filtered away everything, fall back to original results
    if (filteredTop.length === 0) {
      filteredTop = originalTop;
    }
  }

  // keep top 3–5 after filtering
  const top = filteredTop.slice(0, lucky ? 3 : 5);
  const picked = top.map(([i, score]) => ({ ...meta[i], score }));

  if (!top.length || top[0][1] < 0.35) {
    addMsg("I’m not confident this is in our recipes. Try rephrasing or asking for a different option.", "bot");
    statusEl.textContent = "Ready.";
    return;
  }
  ...
  const reply = generated.includes("ASSISTANT:")
    ? generated.split("ASSISTANT:").pop().trim()
    : generated.trim();
